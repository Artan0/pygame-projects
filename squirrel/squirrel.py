...
#Task 1 window width and height 960x480
...
#Task 4
LOSTSIZE = 7
# Additional constants for power-up Task 5
POWERUP_SIZE = 15
POWERUP_DURATION = 5  # in seconds
POWERUP_COLOR = (255, 255, 0)  # Yellow
"""
...
def runGame():
    ...
    gameOverStartTime = 0
    winMode = False
    powerupActive = False  # Track whether the power-up is active
    powerupStart = 0  # Time when the power-up was activated
    ...
    while True:  # main game loop
        ...
        for sObj in squirrelObjs:
            ...
            # Task 3
            sObj["y"] += sObj["movey"] * sObj["bounce_dir"]
            sObj["bounce"] += 1
        ...
        # Task 2 defined logic of view
        playerCenterx = playerObj['x'] + int(playerObj['size'] / 2)
        playerCentery = playerObj['y'] + int(playerObj['size'] / 2)
        if (camerax + HALF_WINWIDTH) - playerCenterx > H_CAMERA:
            camerax = playerCenterx + H_CAMERA - HALF_WINWIDTH
        elif playerCenterx - (camerax + HALF_WINWIDTH) > H_CAMERA:
            camerax = playerCenterx - H_CAMERA - HALF_WINWIDTH
        if (cameray + HALF_WINHEIGHT) - playerCentery > V_CAMERA:
            cameray = playerCentery + V_CAMERA - HALF_WINHEIGHT
        elif playerCentery - (cameray + HALF_WINHEIGHT) > V_CAMERA:
            cameray = playerCentery - V_CAMERA - HALF_WINHEIGHT
        ...
        if powerupActive:
            pygame.draw.rect(DISPLAYSURF, POWERUP_COLOR, (playerObj['x'] - camerax, playerObj['y'] - cameray, POWERUP_SIZE, POWERUP_SIZE))
        ...
        for event in pygame.event.get():  # event handling loop
            if event.type == QUIT:
                ...
                # Activate power-up when 'p' key is pressed
                elif not powerupActive and event.key == K_p:
                    powerupActive = True
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        #Task 4
                        playerObj["size"] -= (int((playerObj["size"] * playerObj["size"]) ** 0.2) + 1)
                        #Task 4
...
def makeNewSquirrel(camerax, cameray):
    ...
    #Task 3
    sq["bounce_dir"] = random.choice([-1, 1])
